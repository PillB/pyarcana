...
has_provenance True`,
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "Patrón habitual: entity —`owns`→ account; account —`transfer`→ account; entity —`has_phone`/`has_email`→ valor de contacto. Preferencia canónica: el valor de contacto como **nodo** intermedio (`E1 → ph:900 ← E2`), no como la arista directa `shared_phone` que aparece en el esquema de arriba. La razón es concreta: si el teléfono es un nodo, se ve de un vistazo cuántas entidades lo comparten — dos es una señal, cuarenta es una centralita. Con aristas directas entre personas ese número queda repartido y hay que reconstruirlo. `shared_phone` se conserva como atajo derivado para consultas rápidas, no como el modelo primario. Una arista directa `shared_phone` entre personas es un atajo derivado, no el modelo primario. En ambos casos es un **hecho de contacto compartido** — no parentesco ni fraude.",
...
detail_kept True`,
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "**Caminos**: se recorren con **BFS** (*breadth-first search*, búsqueda en anchura: explora primero todo lo que está a un salto, luego a dos) o **DFS** (*depth-first search*, en profundidad: sigue un camino hasta el final antes de retroceder). Para evidencia relacional interesa BFS, porque encuentra el camino más corto primero. Ambos con **límite de profundidad** (*hop limit*, máximo de saltos). El path **reproducible** lista nodos en orden estable (vecinos sorted) y, en producción, incluye aristas y evidencia. Sin límite, los caminos largos son caros y poco accionables para la cola humana.",
      ],
      code: {
        language: 'python',
