...
REDACT_AND_QUARANTINE_TRACE`,
    ...
    },
    {
      ...
      subtopicId: "S51-T1-B",
      paragraphs: [
        "**Tokens, costo y latencia** se miden **por etapa** (prompt build, retrieval, generation, tools) y por **percentil** (p50/p95), no solo media. Un p95 de 5 s con media de 200 ms es un incidente de UX, no un «pico normal». El **costo** no sale de una sola fórmula, y suponerlo es la forma habitual de subestimarlo: la generación se cobra por tokens, pero retrieval y tools suelen cobrarse por consulta, por tiempo de ejecución o por almacenamiento, y sus tokens no entran en el contador facturable del modelo. Reconcilia lo que es reconciliable —que los tokens por etapa de modelo sumen `total_tokens`— y suma aparte las etapas que se tarifan de otra forma. Un dashboard que multiplica todo por un precio por token miente hacia abajo. **Redacción** aplica a atributos, eventos, payloads y mensajes de error: un stack trace con email o Authorization es PII en el sink.",
