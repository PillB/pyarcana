  ...
  theory: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "**Orden de los subtemas.** T1 mide el cuello de botella: I/O frente a CPU, el GIL y el coste de serializar. T2 acota la concurrencia —pools, backpressure, rate limits— y añade cancelación y timeouts. T3 hace observable lo que ya corre: logs, métricas, trazas, correlación y SLI/SLO. T4 cierra con la recuperación: estados, checkpoint, idempotencia, reintentos, dead-letter y replay. Primero mides, luego acotas, luego ves, y solo entonces reanudas con criterio.",
...
prefer compact_payload`,
    ...
    },
    {
      ...
      subtopicId: "S38-T2-A",
      paragraphs: [
        "Un pool acota la concurrencia máxima (N workers). Una cola con `maxsize` aplica backpressure: el productor se bloquea o rechaza cuando la cola está llena, en lugar de crecer hasta OOM. En stdlib eso es `queue.Queue(maxsize=Q)` (o `asyncio.Queue` en async): no es un comentario de diseño, es un tope de memoria. **Importante:** `Queue.full()` es solo consultivo (puede cambiar entre la lectura y el `put`); la señal segura de rechazo es atómica con `put_nowait` y `queue.Full`, o un `put(..., timeout=…)`. Un rate limit protege al proveedor mock de un ban o de saturación. El del lab es un **token bucket estático**: reparte una cuota fija de tokens y no los repone, así que conviene ser preciso — no impone una *tasa*, impone un *total*. Un bucket real se recarga por ventana de tiempo, y esa recarga es justamente lo que convierte una cuota en un límite por ventana. Aquí practicamos allow/deny y cola acotada para fijar el modelo mental; el criterio «por ventana» que verás en T4 exige la recarga.",
...
error_budget_action ship_features`,
    ...
    },
    {
      ...
      subtopicId: "S38-T4-A",
      paragraphs: [
        "Un workflow de triage avanza por estados: pending → running → done | failed. Tras cada paso caro (features, score) se escribe un checkpoint **durable** (archivo/SQLite en prod; en el fixture un store que sobrevive al reinicio del proceso). La idempotency key (`case:step:ver`) **identifica la operación, no el intento**: los tres reintentos de un mismo paso comparten la misma key, y por eso el segundo puede reconocerse como repetido. Si la key cambiara en cada intento nunca habría dos iguales y la deduplicación no ocurriría jamás. La semántica exige además un registro atómico que evite reaplicar el side effect si la key ya se vio.",
