...
Probabilidad de churn: 68.5%`,
    ...
    },
    {
      ...
      paragraphs: [
        ...
        '**Experimentación y pruebas A/B.** En 2008, la campaña política de Barack Obama en Estados Unidos realizó un experimento A/B — esto es, una comparación controlada entre dos variantes — para decidir qué versión de la página web de inscripción conseguía más voluntarios. La variante ganadora aumentó las inscripciones en un 40.6%, una diferencia que cambió el rumbo de la elección. Ese episodio ilustró algo que todo analista de datos debe dominar: la diferencia entre observar una diferencia y demostrar que esa diferencia es real.',
        'Una prueba A/B divide aleatoriamente a los usuarios en dos grupos: un grupo de control (versión A) y un grupo de tratamiento (versión B). Después de un período, comparas la métrica de interés — por ejemplo, la tasa de clics — entre ambos grupos. La pregunta estadística es: ¿la diferencia observada es suficientemente grande para descartar el azar? Aquí entran el valor-p (esto es, la probabilidad de observar una diferencia así de grande si en realidad no hay diferencia) y los intervalos de confianza (esto es, el rango dentro del cual estamos seguros de que está el verdadero efecto). Un valor-p menor a 0.05 sugiere que la diferencia es estadísticamente significativa, pero no prueba que sea prácticamente importante.',
        'En Python, `scipy.stats.ttest_ind(grupo_a, grupo_b)` calcula la prueba t de Student para dos muestras independientes. Para proporciones (tasas de clics), usa `statsmodels.stats.proportion.proportions_ztest`. El tamaño de muestra importa: con muy pocos usuarios, puedes no detectar una diferencia real (falso negativo); con demasiados, detectas diferencias minúsculas que no importan (significancia estadística sin relevancia práctica). Calcula el tamaño mínimo con `statsmodels.stats.power`.',
        'La experimentación responsable requiere pre-registro (esto es, declarar tu hipótesis antes de ver los datos), controlar variables de confusión (esto es, factores externos que podrían explicar la diferencia), y evitar el p-hacking (esto es, probar muchas variantes hasta que alguna dé significancia por azar). La reproducibilidad — poder repetir el experimento y obtener el mismo resultado — es la prueba final de que tu hallazgo es genuino.',
      ],
      code: {
        language: 'python',
        title: 'evaluacion.py',
