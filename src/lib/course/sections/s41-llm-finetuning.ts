...
export const section41: CourseSection = {
  ...
  jobRelevance:
    "En equipos de plataforma y producto, las API creadas con FastAPI —un framework web para Python— y sus contratos HTTP convierten decisiones de arquitectura en endpoints versionados con evidencia operativa: respuestas OpenAPI sin PII (información personal identificable), status y errores tipados. Aquí aprendes a garantizar que la misma Idempotency-Key no duplique side effects (cambios observables como crear un job dos veces) y que la lectura conserve compatibilidad con versiones anteriores. Es la base sobre la que luego se monta autorización, schemas estrictos y privacidad por servicio.",
  learningOutcomes: [
