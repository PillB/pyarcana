 ...
 *
 * Renamed from the pre-V3 id "oop" to match what this section actually teaches.
...
export const section05: CourseSection = {
  ...
  tagline: "Funciones con valores por defecto seguros, contratos claros e idempotencia. Con ellas inicias CP-N1-B, el segundo incremento del proyecto acumulativo de Nivel 1.",
  estimatedHours: 9,
  level: "Principiante",
  phase: 0,
  icon: "FunctionSquare",
  ...
  theory: [
    {
      ...
      paragraphs: [
        ...
        "Usa una sola pregunta durante toda la sección: **¿qué promete esta función a quien la llama?** La teoría nombra la promesa y el I Do la hace visible. En el We Do la repararás; en el You Do reunirás cuatro promesas sin mezclarlas. Más adelante empaquetarás el núcleo en una CLI y lo conectarás con archivos o clases. Hoy debe poder probarse sin abrir nada.",
      ],
      callout: {
        type: "tip",
     ...
     },
     {
      ...
      paragraphs: [
        ...
        "**Vocabulario.** Una *función* (`def`) es un bloque reutilizable con nombre de verbo. *`return`* entrega un valor a quien llama; sin `return`, la función devuelve `None`.",
        "Un *objeto mutable* puede cambiar de contenido; las listas y los diccionarios son ejemplos. Si la función podría modificarlo, no lo uses como valor por defecto: usa `None` y crea un objeto nuevo dentro.",
        "Un *orquestador delgado* combina normalizadores sin reimplementar reglas. *LEGB* es el orden en que Python busca un nombre: local, envolvente, global e incorporado. Los parámetros *solo por palabra clave* van después de un `*` y obligan a escribir `nombre=` al llamar.",
...
None`,
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "Los argumentos **posicionales** se vinculan por orden. Los argumentos **por nombre** (`keyword`) se vinculan mediante el nombre escrito en la llamada, como en `fn(x=1)`.",
        "Una lista es un contenedor mutable: su contenido puede cambiar. Python crea una sola vez el objeto usado como valor por defecto, cuando ejecuta la definición. `append` cambia ese mismo objeto. Por eso una llamada puede encontrar elementos que dejó una llamada anterior. Si la función podría modificar una lista o un diccionario, usa `None` y crea un objeto nuevo dentro de cada llamada.",
        "Orden recomendado: primero los parámetros posicionales obligatorios; después, los opcionales con valor por defecto. En llamadas largas, los argumentos por nombre muestran qué valor recibe cada parámetro. Escribir `nombre=` y `email=` evita intercambiar ambos textos sin que Python avise.",
        "Imagina una variante regional: `def normalize_telefono_intl(raw, *, country=\"PE\")`. Allí `country` es **solo por palabra clave** (`keyword-only`). El `*` obliga a escribir `country=` y evita que el país se cuele como segundo argumento posicional.",
        "Este ejemplo solo muestra una firma. No cambia el contrato de `normalize_telefono` en esta sección: esa función todavía deja únicamente los dígitos y no consulta el país. En un ETL de una fintech peruana, escribir `country=` evita que alguien intercambie `raw` y `country` y asigne el código de país equivocado.",
...
# normalize_email("sin-arroba")`,
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "Los **type hints** (`def f(x: str) -> str`) describen los tipos esperados, pero no convierten ni comprueban valores mientras el programa corre. **mypy** es un comprobador estático: revisa el código antes de ejecutarlo. Python no le consulta nada durante la ejecución. Si necesitas validar un dato al recibirlo, debes escribir la comprobación o usar una librería que la realice, como Pydantic.",
        "Los hints ayudan a las personas y a las herramientas a revisar el contrato. En S05 serán **graduales**: anota las funciones públicas de los normalizadores. Los genéricos avanzados y `Protocol` quedan para más adelante.",
...
Atención: Revisar correo`,
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "Antes de refactorizar, fija **ejemplos ejecutables**: `assert normalize_email('A@B.COM') == 'a@b.com'`. Luego cambia la forma interna y vuelve a ejecutarlos. Si siguen verdes, sabes que sobrevivió la conducta cubierta por esos ejemplos. No sabes más que eso.",
        "Un `assert` es una comprobación para desarrollo y ejemplos. Python puede omitirlo cuando ejecuta con `-O`; por eso no debe ser la única validación de datos externos en producción. Sin ejemplos previos, no tienes con qué comparar la conducta anterior y la nueva.",
...
+51999000111`,
        ...
        why:
          "LEGB: la función interna resuelve `prefix` en el ámbito envolvente. Cada fábrica conserva su propia configuración y no altera las demás. Antes de anteponer el prefijo, la demo retira el mismo código si ya venía en la entrada. Así evita duplicarlo y mantiene la idempotencia en los tres casos mostrados. No necesita una clase ni una variable global mutable.",
        retrospective:
