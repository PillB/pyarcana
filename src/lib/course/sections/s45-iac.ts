...
export const section45: CourseSection = {
  ...
  jobRelevance:
    "En equipos de plataforma y producto, cloud, almacenamiento, colas e infraestructura operan el job asíncrono (tarea en segundo plano que corre sin bloquear al llamador) del control plane: object store de artefactos (un PDF de reporte, por clave), estado durable (que sobrevive a reinicios), colas con reintentos y DLQ (cola aparte para mensajes fallidos tras N intentos) e IAM de mínimo privilegio. Aquí aprendes a garantizar que los reintentos no dupliquen resultados y que costo, backup y recuperación estén medidos. El foco es el contrato del job, no aprender un vendor (AWS, GCP o Azure) ni una herramienta de IaC como fin en sí mismo.",
  learningOutcomes: [
...
retriable_truth object + relational`,
      ...
      callout: {
        ...
        content: "Evidencia mínima de S45-T1-A: Si el reintento del job lee status solo desde cache, el ADR de persistencia está roto. La verdad durable es object store + relacional; el TTL del cache no se audita como fuente.",
      },
    },
    {
      ...
      paragraphs: [
        ...
        "En `CASO-IQU-045`: el status relacional del job de reportes se declara *read-after-write*; el object store del artefacto puede ser eventual entre regiones. Un backup sintético **cada 4 h** con restore de 25 min frente a RTO 30 min y RPO 6 h es evidencia de T1-B. La cadencia no es un detalle: es lo que acota el RPO. Un backup diario deja hasta 24 h de escrituras sin copia, así que frente a un RPO de 6 h el propio contrato de arriba lo declararía `DECLARE_DATA_LOSS_RISK` (`backup_age_h > rpo_h`) por muy rápido que restaure — no una captura de consola sin drill medido.",
      ],
      code: {
        language: 'python',
...
restore_breach False`,
      ...
      callout: {
        ...
        content: "Antes de promover S45-T1-B, Un backup sin restore ensayado no es evidencia. Compara `backup_age_h` con RPO y `restore_minutes` con RTO; si el drill falla, declara riesgo de pérdida de datos, no un warning opcional.",
      },
    },
    {
...
no_redeliver_if_acked False`,
      ...
      callout: {
        ...
        content: "La revisión de S45-T2-A conserva que At-least-once + visibility timeout: el ack va **después** del efecto durable. Sin idempotency key, una reentrega puede reimprimir el PDF del reporte sintético.",
      },
    },
    {
...
order per_partition`,
      ...
      callout: {
        ...
        content: "Contrato S45-T2-B: Poison tras N intentos → estado terminal en DLQ con evidencia. Reintentar sin store de dedup produce doble side-effect; borrar el mensaje sin audit trail no es contención.",
      },
    },
    {
      ...
      paragraphs: [
        ...
        "**Contenedores y orquestación.** En 2013, un pequeño startup de San Francisco llamado Docker publicó una herramienta que cambiaria el panorama del despliegue de software para siempre. La idea era simple: en lugar de instalar dependencias directamente en el servidor, empaqueta tu aplicación con todo lo que necesita — intérprete, librerías, variables de entorno — en un contenedor (esto es, una unidad autocontenida y portátil que se ejecuta de forma idéntica en cualquier máquina). Un año después, en 2014, el equipo de Google publicó Kubernetes, un sistema que gestiona miles de contenedores automáticamente: los arranca, los reinicia si fallan, los escala según la demanda y los distribuye entre servidores. Para 2026 ambos son referencias obligadas —el formato de imagen de Docker se estandarizó como OCI y Kubernetes es el orquestador más extendido—, aunque «toda empresa seria usa las dos» sería falso: hay equipos que despliegan en plataformas gestionadas sin tocar Kubernetes, otros que usan otros runtimes de contenedor, y muchos que no necesitan orquestación en absoluto. Lo que sí conviene es entender el modelo, porque es el vocabulario con el que se discute despliegue.",
        "Un contenedor Docker se define con un `Dockerfile` — un archivo de texto que describe paso a paso cómo construir la imagen (esto es, la plantilla inmutable del contenedor). `FROM python:3.12-slim` parte de una imagen base; `COPY . /app` copia tu código; `RUN pip install -r requirements.txt` instala dependencias; `CMD ['python', 'main.py']` define el comando de arranque. La regla de oro es **un proceso por contenedor** y **ejecutar como usuario no-root** (esto es, sin privilegios de administrador) para limitar el daño si un atacante compromete el contenedor. `docker build -t mi-app .` construye la imagen; `docker run -p 8000:8000 mi-app` la ejecuta.",
        "Kubernetes (abreviado K8s) orquesta contenedores a escala. Un **Pod** (esto es, la unidad mínima desplegable) contiene uno o más contenedores. Un **Deployment** (esto es, un controlador) define cuántas réplicas del Pod quieres y cómo se actualizan. Un **Service** (esto es, un equilibrador de carga interno) da una dirección estable para alcanzar los Pods aunque se reinicien. Un **Ingress** describe cómo debería exponerse el Service al exterior. Es una declaración, no una pieza que funcione sola: hace falta un **controlador** instalado en el clúster —NGINX, Traefik o el del proveedor— que lea ese recurso y sea el proxy real, y el HTTPS depende de que además se le den certificados. Un Ingress aplicado en un clúster sin controlador no expone nada y no da error: simplemente no ocurre. El manifiesto se escribe en YAML declarativo: describes el estado deseado y K8s lo mantiene automáticamente. Si un Pod muere, K8s lo reemplaza en segundos. Si el tráfico aumenta, el Horizontal Pod Autoscaler añade réplicas según CPU o métricas personalizadas.",
        "La retrospectiva: los contenedores resuelven el problema de «en mi máquina funciona» al hacer que la máquina sea el contenedor mismo. Kubernetes resuelve el problema de gestionar cientos de contenedores al declarar el estado deseado y dejar que el sistema lo mantenga. La combinación de ambos es lo que permite que un equipo pequeño despliegue y escale servicios con la misma madurez operacional que una gran empresa.",
      ],
      code: {
        language: 'python',
...
network private`,
      ...
      callout: {
        ...
        content: "Para S45-T3-A, Escala por lag de cola cuando el backlog cruza el umbral; con lag bajo observa CPU. Workers por encima de cuota o sin backpressure rompen el SLO antes de que el dashboard lo note.",
      },
    },
    {
...
least_privilege_actions ['object:get', 'queue:ack']`,
      ...
      callout: {
        ...
        content: "Promoción de S45-T3-B: Least privilege se demuestra con allowlist de acciones y hosts, más prueba negativa de `iam:admin` y de un host desconocido. Un print de `least_privilege=True` sin denegaciones no es evidencia.",
      },
    },
    {
...
unexpected_destroy False`,
      ...
      callout: {
        ...
        content: "El dueño de S45-T4-A acepta que Acepta el plan solo si declared == planned, el entorno es dev/staging/prod, no hay secretos en claro y no hay destroys inesperados. Rechazar un plan malo es el contrato; «aplicar y ver» no lo es.",
      },
    },
    {
...
currency PEN`,
      ...
      callout: {
        ...
        content: "Cierre de S45-T4-B: Cierra el lab con alarma de costo y recuperación documentadas, evidencia de `FREEZE_SCALE_OUT` cuando haga falta, y ruta humana `COST_OWNER_REVIEW` si falta export o restore. Montos en PEN sintéticos; riesgo residual y límites del laboratorio con stdlib quedan en el portfolio.",
      },
    },
  ],
  iDo: {
...
` ,
      ...
      },
      {
        ...
        instruction:
          "S45-T2-A-E1 · Salida: debe devolver el PASS del contrato. 1. Starter: PASS si `not acked_after_effect` o key vacía (DEFECT).\n2. Invierte: exige los cinco campos de la política correcta.\n3. Status PASS vs. NACK_AND_RETRY.\n4. Conserva el print.",
...
` ,
      ...
      },
      {
        ...
        instruction:
          "S45-T2-B-E1 · Salida: debe devolver el PASS del contrato. 1. Starter: PASS si len(processed)==len(messages) o no terminal (DEFECT).\n2. Usa igualdad de conjuntos y exige terminal_in_dlq.\n3. Exige ordered_partition.\n4. Conserva print PASS/DEDUP_OR_DLQ.",
...
` ,
      ...
      },
      {
        ...
        instruction:
          "S45-T2-B-E3 · Salida: debe devolver el PASS del contrato. 1. Missing del flag → INSPECT_MESSAGE_ORDER.\n2. Completo: predicado de E1 → CONTINUE o DEDUP_OR_DLQ.\n3. Imprime en orden.\n4. Conserva fixtures.",
...
` ,
      ...
      },
      {
        ...
        instruction:
          "S45-T3-A-E3 · Salida: debe devolver el PASS del contrato. 1. Missing → REQUEST_CAPACITY.\n2. Completo: predicado sano → CONTINUE; roto → APPLY_BACKPRESSURE.\n3. Imprime en orden.\n4. No toques datos.",
...
` ,
      ...
      },
      {
        ...
        instruction:
          "S45-T3-B-E1 · Salida: debe devolver el PASS del contrato. 1. Starter: PASS en denegación (DEFECT).\n2. Invierte a membership + private_path.\n3. Status PASS vs. DENY_IAM_OR_EGRESS.\n4. Conserva print.",
...
` ,
      ...
      },
      {
        ...
        instruction:
          "S45-T3-B-E3 · Salida: debe devolver el PASS del contrato. 1. Missing → REQUEST_SCOPED_POLICY.\n2. Completo: predicado sano → CONTINUE; roto → DENY.\n3. Imprime en orden.\n4. Conserva fixtures.",
...
` ,
      ...
      },
      {
        ...
        instruction:
          "S45-T4-A-E1 · Salida: debe devolver el PASS del contrato. 1. Starter: PASS con secretos o destroys (DEFECT).\n2. Invierte y añade paridad de sets y entorno válido.\n3. Status PASS vs. REJECT_IAC_PLAN.\n4. Conserva print.",
...
assert meets_contract is True` ,
      ...
      },
      {
        ...
        instruction:
          "S45-T4-A-E2 · Salida: debe devolver el PASS del contrato. 1. Schema primero.\n2. Completo: predicado de E1 → PASS o REJECT.\n3. Imprime los tres.\n4. Conserva env `shared` adverso.",
...
` ,
      ...
      },
      {
        ...
        instruction:
          "S45-T4-A-E3 · Salida: debe devolver el PASS del contrato. 1. Missing → REVIEW_DRIFT.\n2. Completo: predicado limpio → CONTINUE; plan malo → REJECT.\n3. Imprime en orden.\n4. Conserva fixtures.",
...
` ,
      ...
      },
      {
        ...
        instruction:
          "S45-T4-B-E1 · Salida: debe devolver el PASS del contrato. 1. Starter: PASS con sobrepresupuesto (DEFECT).\n2. Invierte desigualdades y exige restore + export.\n3. Status PASS vs. FREEZE_SCALE_OUT.\n4. Conserva print.",
...
` ,
      ...
      },
      {
        ...
        instruction:
          "S45-T4-B-E3 · Salida: debe devolver el PASS del contrato. 1. Missing → COST_OWNER_REVIEW.\n2. Completo: predicado sano → CONTINUE; roto → FREEZE_SCALE_OUT.\n3. Imprime en orden.\n4. Conserva fixtures.",
