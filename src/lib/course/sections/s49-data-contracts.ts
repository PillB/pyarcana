...
{'ok': False, 'action': 'COMPACT_AND_CHECKPOINT'}`,
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "Mecanismo de compactación: el conjunto de hechos post-compaction debe conservar las restricciones críticas (`CRITICAL` ⊆ `facts_after` en el lab; el punto de compactar es precisamente que `facts_before` **no** se conserve entero), retención ≤ política (p. ej. 7 días) y `last_known_good` con prefijo `cp-`. Si se pierde `budget`/`no_prod_write` o el LKG está vacío, `RESTORE_LAST_KNOWN_GOOD` o `REVIEW_COMPACTION_LOSS`. Entrada: memoria + política. Salida: memoria compacta + LKG. Error: drop de restricción crítica → no continuar el run.",
