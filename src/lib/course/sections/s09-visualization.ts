...
cause ParseError monto no parseable: 'N/A'`,
        ...
        why: "El chaining con `from e` deja `__cause__` legible en el post mórtem: el on-call ve la capa de dominio (ValidationError + id de fila) y la causa de parse. Los montos van por `Decimal` y `quantize(0.01)`, nunca por `float`. Un `except Exception` genérico ocultaría ambos niveles.",
        retrospective:
...
pii_completa_ausente=True`,
        ...
        why: "Redacta *antes* de formatear el mensaje. `log.exception` une stack y campos (`correlation_id`, `error_class`, email enmascarado) en un solo ERROR path, sin un segundo print del row crudo al canal de Slack de las 02:10.",
        retrospective:
