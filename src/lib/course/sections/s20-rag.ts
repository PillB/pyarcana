...
export const section20: CourseSection = {
  ...
  selfCheck: {
    questions: [
      {
        question: '¿Qué es RAG (Retrieval-Augmented Generation) y por qué se usa?',
        options: [
          'Combina un LLM con un sistema de recuperación de documentos: el LLM responde basándose en documentos específicos recuperados, no solo en su entrenamiento — reduce alucinaciones',
          'Un sistema que genera documentos automáticamente con IA',
          'Un método para entrenar LLMs más rápido',
          'Un protocolo de red para servir modelos de IA',
        ],
        correctIndex: 0,
        explanation: 'RAG recupera documentos relevantes del usuario desde una base vectorial, los inyecta en el prompt del LLM, y el LLM genera una respuesta fundamentada en esos documentos. Esto reduce alucinaciones (el LLM tiene contexto real) y permite actualizar conocimiento sin reentrenar el modelo.',
      },
      {
        question: '¿Qué son los embeddings y cómo se usan en RAG?',
        options: [
          'Representaciones vectoriales de texto que capturan significado semántico — textos similares tienen vectores cercanos, permitiendo búsqueda por similitud en vez de keyword matching',
          'Imágenes embebidas en documentos de texto',
          'Fragmentos de código insertados en el prompt del LLM',
          'Métodos de compresión para reducir el tamaño del modelo',
        ],
        correctIndex: 0,
        explanation: 'Los embeddings convierten texto en vectores numéricos (ej: 1536 dimensiones). Textos con significado similar producen vectores cercanos en el espacio vectorial. En RAG, se calcula el embedding del query del usuario y se buscan los documentos cuyos embeddings son más cercanos (similitud coseno).',
      },
      {
        question: '¿Qué es el chunking en RAG y por qué importa?',
        options: [
          'Dividir documentos largos en fragmentos más pequeños (chunks) para que cada fragmento quepa en el contexto del LLM y la recuperación sea más precisa',
          'Comprimir el modelo LLM para que ocupe menos memoria',
          'Dividir el dataset de entrenamiento en batches',
          'Fragmentar el prompt en múltiples partes',
        ],
        correctIndex: 0,
        explanation: 'Los documentos pueden tener 100+ páginas, pero el LLM tiene un límite de contexto. Chunking divide el documento en fragmentos de 500-2000 tokens con overlap. Cada chunk se embeddea por separado. Cuando el usuario pregunta, se recuperan los chunks más relevantes (no todo el documento), mejorando precisión y reduciendo costo de tokens.',
      },
      {
        question: '¿Cuál es la diferencia entre búsqueda keyword (BM25) y búsqueda vectorial en RAG?',
        options: [
          'BM25 busca coincidencias exactas de palabras; la búsqueda vectorial busca similitud semántica — "gato" y "felino" son cercanos en vectores pero no coinciden en keywords',
          'BM25 es más rápido; la búsqueda vectorial es más precisa',
          'BM25 funciona solo en inglés; la vectorial es multilingüe',
          'No hay diferencia, son dos implementaciones del mismo algoritmo',
        ],
        correctIndex: 0,
        explanation: 'BM25 (keyword search) encuentra documentos que contienen las palabras exactas del query. La búsqueda vectorial encuentra documentos con significado similar, incluso si no comparten palabras. RAG moderno usa hybrid search: combina BM25 + vectorial para obtener lo mejor de ambos — precisión de keywords + comprensión semántica.',
      },
      {
        question: '¿Qué es un vector store y cuáles son las opciones principales?',
        options: [
          'Una base de datos optimizada para almacenar y buscar vectores por similitud — opciones: Pinecone (SaaS), pgvector (PostgreSQL), Chroma (local), FAISS (Facebook)',
          'Un sistema de control de versiones para modelos de ML',
          'Un almacenamiento de objetos como S3 pero para vectores',
          'Una librería de visualización de datos multidimensionales',
        ],
        correctIndex: 0,
        explanation: 'Un vector store almacena embeddings y permite búsqueda por similitud coseno en milisegundos. Pinecone es SaaS gestionado. pgvector añade búsqueda vectorial a PostgreSQL. Chroma es local y ligero para desarrollo. FAISS de Facebook es la librería de referencia para búsqueda vectorial a escala.',
      },
    ],
  },
  resources: {
