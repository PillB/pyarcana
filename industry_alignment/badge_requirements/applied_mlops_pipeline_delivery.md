# Badge Requirements — Applied MLOps Pipeline Delivery

**Badge ID:** `applied_mlops_pipeline_delivery`  
**Version:** 1.0.0  
...
> The learner has independently demonstrated applied MLOps pipeline delivery — model deployment, drift monitoring, retraining triggers, and pipeline observability — through You Do projects and section exams above the provisional floor.
...
- `BADGE:applied_mlops_pipeline_delivery:integrator`
...
`BADGE:applied_mlops_pipeline_delivery:integrator`. The learner must:

1. Hold every prerequisite badge
   (`production_python_delivery_foundations`, `applied_rag_llm_service_development`).
...
3. Complete a bounded independent exercise that ties together
   ...
   specification is in `badge_rubrics/applied_mlops_pipeline_delivery.json#integrator_spec`.
