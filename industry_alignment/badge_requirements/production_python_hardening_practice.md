# Badge Requirements — Production Python Hardening Practice

**Badge ID:** `production_python_hardening_practice`  
**Version:** 1.0.0  
...
> The learner has independently demonstrated production Python hardening — security mindset, observability, cloud-platform deployment, async service design, and performance tuning — through You Do projects and section exams above the provisional floor.
...
> You can harden a Python service for production: apply a security mindset (OWASP Top 10 — the standard list of web-app security risks like injection and broken authentication), instrument it with observability (structured logs in JSON, OpenTelemetry tracing, Prometheus metrics), deploy to a cloud platform (AWS, GCP, or Azure), and tune its performance (profiling, bottleneck-finding, optimization). NOTE: 'Pilot' status because static type-safety (mypy/Pyright) is still a curriculum gap.
...
- `BADGE:production_python_hardening_practice:integrator`
...
`BADGE:production_python_hardening_practice:integrator`. The learner must:
...
3. Complete a bounded independent exercise that ties together
   ...
   specification is in `badge_rubrics/production_python_hardening_practice.json#integrator_spec`.
