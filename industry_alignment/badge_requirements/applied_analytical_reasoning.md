# Badge Requirements — Applied Analytical Reasoning

**Badge ID:** `applied_analytical_reasoning`  
**Version:** 1.0.0  
...
> The learner has independently demonstrated applied analytical reasoning — building Python visualizations, publishing a basic BI-style dashboard, training and evaluating a classical ML model with appropriate metrics — through You Do projects and section exams above the provisional floor.
...
- `BADGE:applied_analytical_reasoning:integrator`
...
`BADGE:applied_analytical_reasoning:integrator`. The learner must:
...
3. Complete a bounded independent exercise that ties together
   ...
   specification is in `badge_rubrics/applied_analytical_reasoning.json#integrator_spec`.
