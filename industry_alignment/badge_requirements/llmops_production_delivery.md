# Badge Requirements — LLMOps Production Delivery

**Badge ID:** `llmops_production_delivery`  
**Version:** 1.0.0  
...
> The learner has independently demonstrated LLMOps production delivery — fine-tuning, graph-RAG, drift monitoring, observability — through You Do projects and section exams above the provisional floor.
...
- `BADGE:llmops_production_delivery:integrator`
...
`BADGE:llmops_production_delivery:integrator`. The learner must:

1. Hold every prerequisite badge
   (`applied_rag_llm_service_development`, `applied_mlops_pipeline_delivery`).
...
3. Complete a bounded independent exercise that ties together
   ...
   specification is in `badge_rubrics/llmops_production_delivery.json#integrator_spec`.
