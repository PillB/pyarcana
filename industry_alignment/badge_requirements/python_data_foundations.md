# Badge Requirements — Python Data Foundations

**Badge ID:** `python_data_foundations`  
**Version:** 1.0.0  
...
> The learner has independently demonstrated bounded Python fundamentals — interpreter setup, version control with Git, modules and functions, idiomatic Python patterns, OOP basics, and a working test discipline — by passing section exams and completing section You Do projects above the provisional floor.
...
- `BADGE:python_data_foundations:integrator`
...
`BADGE:python_data_foundations:integrator`. The learner must:

1. Hold every prerequisite badge
   (none — entry-point badge).
...
3. Complete a bounded independent exercise that ties together
   ...
   specification is in `badge_rubrics/python_data_foundations.json#integrator_spec`.
