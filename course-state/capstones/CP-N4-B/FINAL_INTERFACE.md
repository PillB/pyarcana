...
**Interfaz:** `platform.deploy(model) -> DeployRecord`

Recibe model. Devuelve DeployRecord con: lineage (esto es, el linaje: el registro de qué dato derivó de qué otro), approval, canary_result, SLO (esto es, un objetivo de nivel de servicio: la promesa medible de qué tan disponible o rápido debe estar el sistema), rollback_available, rollback_proven, train_serve_consistent.
